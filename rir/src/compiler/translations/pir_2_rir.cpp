  ...
  public:
    ...
    void computeLiveness(bool verbose = true) {
        ...
        while (!todo.empty()) {
            ...
            // keep track of currently live variables to update the liveOut
            // sets of my predecessors
            std::set<Value*> accumulated;
            std::map<BB*, std::set<Value*>> accumulatedPhiInput;

            // Mark all incoming live variables
            for (auto v : liveAtEnd[bb]) {
                assert(livenessInterval.count(v));
                if (!livenessInterval[v][bb->id].live ||
                    livenessInterval[v][bb->id].end < bb->size()) {
                    livenessInterval[v][bb->id].live = true;
                    livenessInterval[v][bb->id].end = bb->size();
            ...
            if (!bb->isEmpty()) {
                ...
                do {
                    ...
                    auto markIfNotSeen = [&](Value* v) {
                        ...
                        if (!livenessInterval[v][bb->id].live) {
                            livenessInterval[v][bb->id].live = true;
                            livenessInterval[v][bb->id].end = pos;
                            return true;
                        }
                        return false;
                    };

                    // Update liveness interval end if we haven't seen the
                    // argument yet
                    if (phi) {
                    ...
                    // Update liveness interval begin (due to ssa we know that
                    // liveness begin == declaration site)
                    if (accumulated.count(i)) {
                        assert(livenessInterval.count(i));
                        assert(livenessInterval[i][bb->id].live);
                        livenessInterval[i][bb->id].begin = pos;
            ...
            auto markLiveEntry = [&](Value* v) {
                assert(livenessInterval.count(v));
                assert(livenessInterval[v][bb->id].live);
                livenessInterval[v][bb->id].begin = 0;
            };
    ...
    BreadthFirstVisitor::run(code->entry, [&](BB* bb) {
        ...
        */
        ...
        for (auto it = bb->begin(); it != bb->end(); ++it) {
            auto instr = *it;
            //            if (alloc.stackManipulation.count(instr))
            //                cs << alloc.stackManipulation.at(instr);
            switch (instr->tag) {
            case Tag::LdConst: {
